...
export const Hero: React.FC<HeroProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    gs.to(monsterRef.current, {
      scrollTrigger: {
        ...
        scrub: 1, // Smoothing scrub helps lagginess
      },
      y: 120,
      scale: 1.15,
      opacity: 0,
  ...
  return (
    <SectionWrapper 
      ...
      animate={false}
    >
      {/* 1. Ash / Spores - Performance optimized */}
      {Array.from({ length: 20 }).map((_, i) => (
        <div
          key={i}
          className="absolute w-1 h-1 md:w-1.5 md:h-1.5 bg-gray-400/40 rounded-full animate-ash-drift pointer-events-none z-20"
          style={{
            top: `${Math.random() * 100}%`,
      ...
      {/* 3. Shadow Monster (Mind Flayer) Background - will-change added */}
      <div ref={monsterRef} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] opacity-60 pointer-events-none z-0 will-change-transform">
         <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#2a0505] blur-[100px] animate-pulse-slow mix-blend-screen"></div>
         {[0, 60, 120, 180, 240, 300].map((deg, i) => (
            <div 
              key={i} 
              className="absolute top-1/2 left-1/2 w-48 h-[600px] bg-gradient-to-b from-gray-800/40 via-red-900/20 to-transparent origin-top blur-3xl" 
              style={{ 
      ...
      {/* 4. ELEVEN - will-change added */}
      <div ref={elevenRef} className="absolute top-20 right-[5%] md:right-[15%] z-20 pointer-events-none hidden md:block animate-float will-change-transform">
      ...
      {/* 5. DEMOGORGON - will-change added */}
      <div ref={demoRef} className="absolute top-10 left-[2%] md:left-[10%] z-10 pointer-events-none opacity-60 hidden md:block will-change-transform">
         <div className="w-80 h-80 animate-pulse-slow">
            <svg viewBox="0 0 200 200" className="w-full h-full fill-[#1a0505] drop-shadow-[0_0_10px_rgba(50,0,0,0.8)]">
               <path d="M100,50 Q120,20 140,50 T180,90 Q190,120 160,140 T100,180 Q40,140 20,120 T60,50 Q80,20 100,50 Z" />
      ...
      {/* 6. Bike Group */}
      <div className="absolute bottom-20 md:bottom-32 left-0 w-full z-10 pointer-events-none overflow-hidden h-40 will-change-transform">
         ...
         <div className="animate-bike-ride absolute left-0 bottom-4 flex gap-12 opacity-80">
             <svg width="80" height="60" viewBox="0 0 100 80" className="text-gray-900 fill-current drop-shadow-[0_0_5px_rgba(255,9,0,0.8)]">
             ...
             <svg width="80" height="60" viewBox="0 0 100 80" className="text-gray-900 fill-current drop-shadow-[0_0_5px_rgba(255,9,0,0.8)] transform scale-90 translate-y-2">
                <g transform="translate(0, 0)">
                   <circle cx="20" cy="60" r="12" fill="none" stroke="currentColor" strokeWidth="3" />
                   <circle cx="60" cy="60" r="12" fill="none" stroke="currentColor" strokeWidth="3" />
                   <path d="M20 60 L35 45 L45 60 L60 60 M35 45 L30 35" stroke="currentColor" strokeWidth="3" fill="none" />
      ...
      <div className="absolute top-20 left-10 w-96 h-96 bg-stranger-red/5 rounded-full blur-[120px] pointer-events-none animate-flicker z-0"></div>
      
      {/* Main Content - will-change added */}
      <div ref={contentRef} className="relative z-30 text-center max-w-4xl mx-auto mb-16 px-4 will-change-transform">
        <motion.div 
      ...
      <div className="w-full overflow-hidden pb-12 z-30 relative">
        <div className="flex gap-6 animate-float items-center justify-center">
           {PLACEHOLDER_IMAGES.map((img, idx) => (
             <motion.div 
                key={idx}
                className={`relative rounded overflow-hidden border border-stranger-red/20 group cursor-pointer bg-black ${idx === 2 ? 'w-64 h-96 z-10 shadow-[0_0_50px_rgba(255,9,0,0.3)]' : 'w-48 h-72 opacity-60 hover:opacity-100 transition-opacity'}`}
                whileHover={{ scale: 1.05 }}
             >
                <img src={img} alt="AI Generated" className="w-full h-full object-cover sepia-[0.2] hover:sepia-0 transition-all duration-500" />
